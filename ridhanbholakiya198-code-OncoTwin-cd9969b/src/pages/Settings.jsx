    ...
    <div className="space-y-4 max-w-3xl">
      <Card title="AI providers — bring your own key">
        ...
        <div className="space-y-3">
          {PROVIDERS.map((p) => (
            <div key={p.id} className="flex items-center gap-3 max-sm:flex-col max-sm:items-stretch">
              <label className="w-20 text-sm text-ink shrink-0 max-sm:w-auto">{p.label}</label>
              <input
                type="password"
                value={keys[p.id] || ''}
      ...
      <Card title="Usage limit">
        ...
        <input
          ...
          className="w-32 bg-surface2 border border-border rounded px-3 py-2 text-sm text-ink max-sm:w-full"
        />
      ...
      <Card title="Multi-agent model assignment">
        ...
        <div className="space-y-2">
          {AGENT_DEFS.map((a) => (
            <div key={a.id} className="flex items-center gap-3 max-sm:flex-col max-sm:items-stretch">
              <label className="w-44 text-sm text-ink shrink-0 max-sm:w-auto">{a.name}</label>
              <select
